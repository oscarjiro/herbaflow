...
- ChEMBL documentation: assay_confidence_score scale 0–9; ≥6 = directly-assigned protein target, ≥7 = curated single-protein target.
...
DB cache path emitted `"score": score`; API fallback path emitted `"association_score": t.score`. Frontend `DiseaseTargetResult.association_score` only matched API fallback. DB-sourced targets (the common case) always showed '—' in the score column.  
...
Source column rendered `"db_cache"` and `"open_targets_api"` as literal label strings.  
...
**Citations**:
- Rivals I et al. (2007). *Enrichment or depletion of a GO category within a class of genes.* Bioinformatics 23:401–407.
...
| Score scale | `score` field from STRING JSON = 0–1; `required_score=int(conf*1000)` converts correctly | ✅ |
...
`combined_score` (0–1 float) stored as `weight` in edge data. Stylesheet used `'width': 'data(weight)'` → edges 0.4–1.0px (invisible).  
Fix: Changed to `'width': 'mapData(weight, 0, 1, 1, 6)'` — Cytoscape linear mapping, 0→1px, 1→6px.
...
`degrees = dict(G.degree())` returns raw edge count (integer, unbounded). Column `degree_centrality` implies normalized Freeman form: `C_D(v) = deg(v)/(n-1)`.  
Fix: replaced with `degree_centrality = nx.degree_centrality(G)` → 0-1 float.
...
**Citations**:
- Raudvere U et al. (2019). *g:Profiler: a web server for functional enrichment analysis.* Nucleic Acids Res 47:W191–W198.
...
Background was Stage 5 overlap genes (~10–50 genes), inflating significance. Correct background: all compound targets from Stage 3 (the study's protein universe per Rivals et al. 2007).  
Fix: background now reads `stage3.get("target_gene_symbols")`.
...
| TargetRanking null suppression | `ConfigDict(exclude_none=True)` already present | No action needed |
| Disease display names | Lowercase from Open Targets — display issue | Deferred to T2.7 |
...
|------|--------|-----------|
| T0.1 Stage 2 ADME | ✅ | 3 bugs: ADME fields missing, NP exception label, filter logic |
...
| T0.5 Stage 6 PPI | ✅ | 1 bug: edge width invisible (mapData fix) |
| T0.6 Stage 7 Hub Genes | ✅ | 1 critical bug: raw degree used instead of normalized (Freeman 1979) |
| T0.7 Stage 8 Enrichment | ✅ | 1 scientific bug: wrong background set (overlap → compound targets) |
| T0.8 Misc | ✅ | Stage 1 header rename; all other items confirmed clean or not applicable |

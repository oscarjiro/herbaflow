...
All primary keys are **bare UUID v5 strings** generated by `stable_id(NAMESPACE, key)` from `etl/shared/utils.py`. No entity type prefixes (`pl_`, `tgt_`, etc.) are used — the column name (`plant_id`, `target_id`) provides type context in the schema.

| Entity | Namespace derivation | Input key |
|---|---|---|
| `plant_id` | `uuid.uuid5(NAMESPACE_DNS, "herbaflow.plants")` | `gbif_accepted_usage_key` |
| `compound_id` | `uuid.uuid5(NAMESPACE_DNS, "herbaflow.compounds")` | InChIKey or canonical key |
| `target_id` | `uuid.uuid5(NAMESPACE_DNS, "herbaflow.targets")` | `uniprot:{acc}` or `gene:{symbol}` |
| `compound_target_id` | `uuid.uuid5(NAMESPACE_DNS, "herbaflow.compound_targets")` | `{compound_id}:{target_id}` |
| `disease_id` | `uuid.uuid5(NAMESPACE_DNS, "herbaflow.diseases")` | ontology ID (DOID/MeSH) |
| `disease_target_id` | `uuid.uuid5(NAMESPACE_DNS, "herbaflow.disease_targets")` | `{disease_id}:{target_id}` |

IDs are deterministic: same input always produces the same UUID. Namespace constants live in each ETL module's `utils.py` (e.g. `TARGET_NS` in `etl/disease_targets/utils.py`). The backend replicates `TARGET_NS` in `backend/analysis/stages/stage3_targets.py` since it cannot import from `etl/`.

> **Migration note:** Existing plant rows in the database carry the old `pl_` prefix format. These will be replaced when the plants ETL pipeline is re-run with `load.py --reset` (Phase 6 checklist). All other entity IDs already use bare UUIDs.

...
DEFAULT_INPUT = Path("../../knapsack/out/plants.csv")
DEFAULT_OUTPUT_DIR = Path("out")
DEFAULT_OUTPUT_FILE = "stg_plants.csv"
DEFAULT_LOG_FILE = "extract_plants.log"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Directory where staging outputs should be written (default: 02_extract/out)",
    )
    parser.add_argument(
        "--output-file",
        type=str,
...
def setup_logging(output_dir: Path, log_file: str) -> Path:
    ...
    # Avoid duplicate handlers when rerunning in the same interpreter.
    if logger.handlers:
...
def main() -> int:
    ...
    logging.info("Starting 02_extract")

...
DEFAULT_INPUT = Path("../02_normalize_taxonomy/out/normalized_plants.csv")
DEFAULT_CACHE_DIR = Path("out/cache")
DEFAULT_OUTPUT = Path("out/gbif_matches.csv")
DEFAULT_LOG_FILE = "match_gbif.log"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to normalized input CSV (default: 02_normalize_taxonomy/out/normalized_plants.csv)",
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=DEFAULT_CACHE_DIR,
        help="Directory for cached GBIF JSON responses (default: 03_match_gbif/out/cache)",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=DEFAULT_OUTPUT,
        help="Path to output CSV (default: 03_match_gbif/out/gbif_matches.csv)",
    )
    parser.add_argument(
        "--log-file",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Maximum retry attempts for transient failures (default: 5)",
    )
    parser.add_argument(
        "--backoff-base",
        type=float,
    ...
    parser.add_argument(
        ...
        help="Maximum backoff seconds for retries (default: 60)",
    )
    return parser.parse_args()


def setup_logging(output_path: Path, log_file: str) -> Path:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    log_path = output_path.parent / log_file
...
def main() -> int:
    args = parse_args()
    log_path = setup_logging(args.output, args.log_file)
    logging.info("Starting 03_match_gbif")
    ...
    try:
        ...
        logging.info("Loaded %d input rows from %s", len(df), args.input)
        logging.info("Built %d unique GBIF queries", len(query_table))
        ...
        logging.info("Wrote match table: %s", args.output)
        logging.info(
